...
- `shared/` - UI kit (`shadcn/ui`), the Axios instance, i18n setup, theme store, utilities, and
  `ui/map` (the only module importing `leaflet`/`react-leaflet` - always loaded via
  `React.lazy`/dynamic `import()`, never in an initial route chunk).
