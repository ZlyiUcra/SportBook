...
- [x] T010 Run `quickstart.md` scenarios 1-4 end-to-end against a locally running stack, plus
      non-regression: `yarn test` (all green), `yarn build` (clean, no new dependency, initial route
      chunk unchanged - the map stack stays lazy-loaded, 003 SC-006). Frontend scenarios verified
      end to end in a real headless Chromium (Playwright, added and removed as a one-off dev
      dependency, user-approved 2026-07-20): registered a user, picked Kyiv, zoomed in twice and
      panned, confirmed the narrowed visible-venue count matched the rendered cards exactly (US2),
      opened a venue and navigated back to confirm the exact same count/card set was restored (US1,
      not the default full framing), picked a different city (Vyshhorod) to confirm the viewport
      reset to the full framing, then zoomed in again and changed only the sport filter to confirm
      the viewport did NOT reset (the `keepPreviousData` fix holds up in a real browser), and
      confirmed a full reload clears the session (and with it the in-memory search state) entirely.
      Throwaway test accounts created during the run were deleted afterward. No defects found
