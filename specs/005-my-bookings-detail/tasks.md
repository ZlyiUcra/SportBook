...
- [ ] T004 [P] [US1] Integration test: `GET /api/bookings` and `GET /api/venues/{id}/bookings`
...
- [ ] T005 [P] [US1] Frontend: extend the `Booking` type in
...
- [ ] T006 [US1] Frontend: render the venue name, city (localized via `cityName`), sport, and court
...
- [ ] T007 [US1] Frontend test: a booking row shows venue/city/sport/court alongside time, status,
...
- [ ] T008 [P] [US2] Unit test: the `BookingStatusFilter` predicate maps each choice to the right
...
- [ ] T009 [P] [US2] Unit test: `ToQueryString()` proves the status predicate plus the
      court->venue->city Include translate to SQL (no client evaluation) in
      `backend/tests/SportBook.UnitTests/BookingStatusFilterTests.cs`

### Implementation for User Story 2

- [ ] T010 [US2] Add a `BookingStatusFilter` enum (All/Upcoming/Completed/Cancelled) in
...
- [ ] T011 [US2] Add the optional `status` query parameter (default All) to `GET /api/bookings` in
...
- [ ] T012 [P] [US2] Integration test: `status=Upcoming|Completed|Cancelled` each return only their
...
- [ ] T013 [US2] Frontend: add the optional `status` arg to `listMyBookings` in
...
- [ ] T014 [US2] Frontend test: selecting each filter requests that `status` and shows the matching
...
- [ ] T015 [US3] Frontend: Prev/Next pagination on
      ...
      on T013)
- [ ] T016 [US3] Frontend test: Prev/Next move pages and disable at the ends, and changing the
...
- [ ] T017 [P] Response-DTO audit for `BookingResponse` - confirm only venue/city/sport/court
      labels were added and no owner id or other internal field leaks (contract MUST, spec FR-011)
- [ ] T018 [P] Update root `README.md` "Using the application" (My bookings now shows venue/sport
